...
class Find_Bigram(minimum_edit_distance.Minimum_Edit_Distance):
	...
	for correct_words in word_unigram_prob:
		...
		if  0< med <= def_med:
			...
			prv_score = bigram_probs.get((previous_word, correct_words), 1) * word_unigram_prob[correct_words]
			next_score = bigram_probs.get((correct_words, next_word), 1) * word_unigram_prob[correct_words]
